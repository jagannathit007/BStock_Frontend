...
import { DollarSign, MessageSquare, Send, X, Clock, CheckCircle, User, Package, History } from 'lucide-react';
...
const BiddingForm = ({ product, isOpen, onClose, onSuccess }) => {
  ...
  const fetchProductBids = async () => {
    setBidsLoading(true);
    try {
      const response = await NegotiationService.getProductBids(product.id, 1, 50);
  ...
  const hasNewerNegotiation = (currentNegotiation, bidGroup) => {
    ...
    return bidGroup.bids.some(bid => {
      ...
      return bid.bidId === currentNegotiation.bidId &&
  ...
  const canMakeCounterForBid = (bidGroup) => {
    ...
    const hasAdminCounter = bidGroup.bids.some(bid => bid.FromUserType === 'Admin');
  ...
  const getProductImage = (productId) => {
    ...
    return productId?.skuFamilyId?.images?.[0] ? `${import.meta.env.VITE_BASE_URL}/${productId?.skuFamilyId?.images[0]}` : '';
  };
  ...
  const groupBidsByBidId = (bids) => {
    ...
    return Object.values(grouped).sort((a, b) => {
      const aLatest = Math.max(...a.bids.map(bid => new Date(bid.createdAt).getTime()));
      const bLatest = Math.max(...b.bids.map(bid => new Date(bid.createdAt).getTime()));
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Product Bidding</h2>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="flex border-b border-gray-200">
          <button
            ...
            className={`px-6 py-3 font-medium text-sm transition-colors flex items-center space-x-2 ${
              activeTab === 'history'
            ...
            <div className="space-y-4">
              {bidsLoading ? (
                ...
                groupedBids.map((bidGroup) => (
                  <div key={bidGroup.bidId} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    {/* Group Header */}
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          {bidGroup.status === 'accepted' ? 'Accepted' : 'Negotiating'}
                        </span>
                    ...
                    <div className="space-y-2">
                      {bidGroup.bids
                        ...
                        .map((bid) => (
                          <div key={bid._id} className="bg-gray-50 rounded-lg p-3 border-l-4 border-blue-500">
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">No Bidding History</h3>
                  <p className="text-gray-500 max-w-md">
                    There are no bids for this product yet. Be the first to make an offer!
                  </p>
                </div>
              )}
            </div>
