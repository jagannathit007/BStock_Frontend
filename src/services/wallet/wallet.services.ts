...
export class WalletService {
  static async getWallet(): Promise<WalletResponse> {
    try {
      ...
      toastHelper.showTost(res.data?.message || (ok ? 'Wallet retrieved successfully' : 'Failed to fetch wallet'), ok ? 'success' : 'error');
      return res.data;
    } catch (err: any) {
  ...
  static async listTransactions(page: number = 1, limit: number = 10): Promise<TransactionListResponse> {
    try {
      ...
      const ok = res.data?.success === true || res.data?.status === 200;
      if (!ok) {
        toastHelper.showTost(res.data?.message || 'Failed to fetch transactions', 'error');
      }
      return res.data;
    } catch (err: any) {
